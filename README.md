...
| `maxSessionQueries` | No | unlimited | Max conversation rounds sent to LLM (context window) |
...
### Attachment generation check (`X-Expected-Container-Id`)

Chat attachments are anchored to the uploading runtime's container generation. `POST /v1/files/uploads`, `GET /v1/files/content`, and attachment-bearing run requests accept an optional `X-Expected-Container-Id: <full 64-hex containerId>` header; when present, the runtime atomically verifies its own container identity **before any attachment I/O or run start** — mismatch → `412 generation_mismatch` (zero I/O, no SSE flush, no assistant message), undeterminable identity → `503 generation_unavailable` (never ignored). Omitting the header keeps the legacy behavior. Identity resolution prefers an explicit `ZERONE_RUNTIME_CONTAINER_ID` (deployer injection), then the cgroup full ID, then the Docker-default 12-hex hostname prefix (full 64-hex expected compared on its first 12 chars only); explicit/invalid/contradictory sources fail closed. `GET /health` declares `capabilities.attachmentExpectedGeneration: true` for capability probing. **Upgrade order:** publish this runtime → Hub enables capability probing and sends the header → redeploy agents.
